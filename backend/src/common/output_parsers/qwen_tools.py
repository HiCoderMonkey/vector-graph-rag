    ...
    def parse_result(self, result: List[Generation], *, partial: bool = False) -> Any:
        ...
        for res in json_results:
            try:
                ...
                if not isinstance(res.get("args",{}).get("properties",None), dict):
                    raise ValueError(
                        f"Tool arguments must be specified as a dict, received: "
                        f"{res.get('args',{}).get('properties',None)}"
                    )
                pydantic_objects.append(name_dict[res["type"]](**res["args"]["properties"]))
